...
func AddProductToCart(ctx context.Context, prodCollection *mongo.Collection, userCollection *mongo.Collection, productID primitive.ObjectID, userID string) error {
	...
	searchdb, err := prodCollection.Find(ctx, bson.M{"_id": productID})
	...
	update := bson.D{{Key: "$push", Value: bson.D{primitive.E{Key: "usercart", Value: bson.D{{Key: "$each", Value: productlist}}}}}}
...
func RemoveCartItem(ctx context.Context, userCollection *mongo.Collection, productcollection *mongo.Collection, productID primitive.ObjectID, userID string) error {
	usert_id, err := primitive.ObjectIDFromHex(userID)
	if err != nil {
		return ErrUserIDIsNotValid
	}
